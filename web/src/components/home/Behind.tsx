...
export function Behind({ src, frame }: { src?: string; frame?: Frame | null }) {
  const t = useTranslations("home.behind");
  const c = useTranslations("cta");
  return (
    <section className="bg-dark text-on-dark">
      <div className="grid lg:grid-cols-2">
        <Reveal className="min-h-[420px] lg:min-h-[640px]" style={boxStyle(frame)}>
          <Photo src={src} seed="behind-main" frame={frame && { ...frame, fit: "cover" }} sizes="(min-width:1024px) 50vw, 100vw" className="h-full w-full" />
        </Reveal>
