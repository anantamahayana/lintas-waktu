...
export function InvoiceDoc({ doc, business, className }: { doc: Doc; business: Business; className?: string }) {
  ...
  const stamp = doc.status === "paid" ? "Paid" : doc.status === "void" ? "Void" : doc.kind === "quote" ? null : t.balance > 0 && t.deposit_paid > 0 ? "Balance due" : null;
    ...
    <article className={clsx("invoice-doc relative bg-white text-ink w-full max-w-[820px] mx-auto px-8 py-10 sm:px-14 sm:py-16 border border-line", className)}>
      ...
      <div className="mt-12 grid sm:grid-cols-[1fr_auto] gap-8 items-end">
        <div className="flex flex-col gap-1">
          <span className="t-mono text-mute">{title} for</span>
        ...
        <dl className="grid grid-cols-[auto_auto] gap-x-6 gap-y-1.5 t-small sm:text-right">
          <dt className="t-mono text-faint">{title} no.</dt><dd className="font-mono">{doc.number}</dd>
          <dt className="t-mono text-faint">Issued</dt><dd>{fmtDate(doc.issued_at)}</dd>
          {doc.due_at && doc.kind !== "quote" && <><dt className="t-mono text-faint">Due</dt><dd>{fmtDate(doc.due_at)}</dd></>}
          {doc.due_at && doc.kind === "quote" && <><dt className="t-mono text-faint">Valid until</dt><dd>{fmtDate(doc.due_at)}</dd></>}
          {doc.paid_at && <><dt className="t-mono text-faint">Paid</dt><dd>{fmtDate(doc.paid_at)}</dd></>}
        </dl>
      </div>
      ...
      <table className="mt-12 w-full border-t border-ink">
        <thead>
          <tr className="t-mono text-faint text-left">
            ...
            <th className="py-3 font-normal text-right w-14 hidden sm:table-cell">Qty</th>
            <th className="py-3 font-normal text-right w-36 hidden sm:table-cell">Unit</th>
            <th className="py-3 font-normal text-right w-32 sm:w-40">Amount</th>
          </tr>
        </thead>
        <tbody>
          {doc.items.map((it, i) => (
      ...
      <div className="mt-6 flex justify-end">
        <dl className="w-full sm:w-[340px] grid grid-cols-[1fr_auto] gap-y-2 t-small tabular-nums">
          {(t.discount > 0 || t.tax > 0) && <><dt className="text-mute">Subtotal</dt><dd className="text-right">{money(t.subtotal, doc.currency)}</dd></>}
          {t.discount > 0 && <><dt className="text-mute">Discount</dt><dd className="text-right">− {money(t.discount, doc.currency)}</dd></>}
          {t.tax > 0 && <><dt className="text-mute">Tax {doc.tax_percent}%</dt><dd className="text-right">{money(t.tax, doc.currency)}</dd></>}
          <dt className="border-t border-ink pt-3 font-serif text-[20px]">Total</dt><dd className="border-t border-ink pt-3 text-right font-serif text-[20px]">{money(t.total, doc.currency)}</dd>
          {t.deposit_paid > 0 && doc.status !== "paid" && (
            <>
              <dt className="text-mute">Received</dt><dd className="text-right">− {money(t.deposit_paid, doc.currency)}</dd>
              <dt className="font-medium">Balance due</dt><dd className="text-right font-medium">{money(t.balance, doc.currency)}</dd>
            </>
          )}
        </dl>
      </div>
      ...
      <footer className="mt-14 grid sm:grid-cols-2 gap-8 t-small text-mute">
        {business.bank_details && doc.kind !== "quote" && doc.status !== "paid" && (
          <div className="flex flex-col gap-1.5">
            <span className="t-mono text-faint">Payment</span>
